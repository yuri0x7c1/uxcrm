...
public class OldPartyRateBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public OldPartyRateBaseService(ExecuteFindService executeFindService) {
	 ...
	 * Find OldPartyRates
	 */
	public List<OldPartyRate> find(Integer start, Integer number,
	...
	/**
	 * Find one OldPartyRate
	 */
	public Optional<OldPartyRate> findOne(Object partyId, Object rateTypeId,
			Object currencyUomId, Object fromDate) {
		List<OldPartyRate> entityList = null;
		In in = new In();
		in.setEntityName(OldPartyRate.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("partyId", EntityOperator.EQUALS,
						...
						"currencyUomId", EntityOperator.EQUALS, currencyUomId),
	 ...
	 * Get party
	 */
	public Optional<Party> getParty(OldPartyRate oldPartyRate) {
		List<Party> entityList = null;
		In in = new In();
		in.setEntityName(Party.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("partyId", EntityOperator.EQUALS,
						oldPartyRate.getPartyId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
				entityList = Party
	 ...
	 * Get currency uom
	 */
	public Optional<Uom> getCurrencyUom(OldPartyRate oldPartyRate) {
		List<Uom> entityList = null;
		In in = new In();
		in.setEntityName(Uom.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("uomId", EntityOperator.EQUALS,
						oldPartyRate.getCurrencyUomId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
