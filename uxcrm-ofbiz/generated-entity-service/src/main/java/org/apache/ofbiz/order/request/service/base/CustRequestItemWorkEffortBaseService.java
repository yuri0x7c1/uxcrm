...
public class CustRequestItemWorkEffortBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public CustRequestItemWorkEffortBaseService(
			ExecuteFindService executeFindService) {
		this.executeFindService = executeFindService;
	}

	/**
	 * Count CustRequestItemWorkEfforts
	 */
	...
	/**
	 * Find CustRequestItemWorkEfforts
	 */
	public List<CustRequestItemWorkEffort> find(Integer start, Integer number,
	...
	/**
	 * Find one CustRequestItemWorkEffort
	 */
	public Optional<CustRequestItemWorkEffort> findOne(Object custRequestId,
			Object custRequestItemSeqId, Object workEffortId) {
		List<CustRequestItemWorkEffort> entityList = null;
		In in = new In();
		in.setEntityName(CustRequestItemWorkEffort.NAME);
	...
	public Optional<CustRequestItem> getCustRequestItem(
			CustRequestItemWorkEffort custRequestItemWorkEffort) {
		List<CustRequestItem> entityList = null;
		In in = new In();
		in.setEntityName(CustRequestItem.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays.asList(
				new EntityExpr("custRequestId", EntityOperator.EQUALS,
						custRequestItemWorkEffort.getCustRequestId()),
				new EntityExpr("custRequestItemSeqId", EntityOperator.EQUALS,
						custRequestItemWorkEffort.getCustRequestItemSeqId())),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	...
	public Optional<WorkEffort> getWorkEffort(
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("workEffortId", EntityOperator.EQUALS,
						custRequestItemWorkEffort.getWorkEffortId())),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
