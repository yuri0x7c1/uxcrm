...
public class VisualThemeBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public VisualThemeBaseService(ExecuteFindService executeFindService) {
	 ...
	 * Find VisualThemes
	 */
	public List<VisualTheme> find(Integer start, Integer number,
	...
	/**
	 * Find one VisualTheme
	 */
	public Optional<VisualTheme> findOne(Object visualThemeId) {
		List<VisualTheme> entityList = null;
		In in = new In();
		in.setEntityName(VisualTheme.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("visualThemeId", EntityOperator.EQUALS,
						visualThemeId)), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
				entityList = VisualTheme.fromValues(out.getListIt()
	...
	public List<VisualThemeResource> getVisualThemeResources(
			VisualTheme visualTheme, Integer start, Integer number,
			List<String> orderBy) {
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("visualThemeId", EntityOperator.EQUALS,
						visualTheme.getVisualThemeId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
