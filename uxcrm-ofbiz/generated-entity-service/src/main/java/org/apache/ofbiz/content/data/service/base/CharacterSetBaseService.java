...
public class CharacterSetBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public CharacterSetBaseService(ExecuteFindService executeFindService) {
	 ...
	 * Find CharacterSets
	 */
	public List<CharacterSet> find(Integer start, Integer number,
			List<String> orderBy, EntityConditionList conditions) {
		List<CharacterSet> entityList = Collections.emptyList();
		In in = new In();
		in.setEntityName(CharacterSet.NAME);
		if (start == null) {
	...
	/**
	 * Find one CharacterSet
	 */
	public Optional<CharacterSet> findOne(Object characterSetId) {
		List<CharacterSet> entityList = null;
		In in = new In();
		in.setEntityName(CharacterSet.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("characterSetId", EntityOperator.EQUALS,
						characterSetId)), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
				entityList = CharacterSet.fromValues(out.getListIt()
	 ...
	 * Get contents
	 */
	public List<Content> getContents(CharacterSet characterSet, Integer start,
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("characterSetId", EntityOperator.EQUALS,
						characterSet.getCharacterSetId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	 ...
	 * Get data resources
	 */
	public List<DataResource> getDataResources(CharacterSet characterSet,
