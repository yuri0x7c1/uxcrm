...
public class WorkReqFulfTypeBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public WorkReqFulfTypeBaseService(ExecuteFindService executeFindService) {
		this.executeFindService = executeFindService;
	}

	/**
	 * Count WorkReqFulfTypes
	 */
	...
	/**
	 * Find WorkReqFulfTypes
	 */
	public List<WorkReqFulfType> find(Integer start, Integer number,
			List<String> orderBy, EntityConditionList conditions) {
		List<WorkReqFulfType> entityList = Collections.emptyList();
		In in = new In();
		in.setEntityName(WorkReqFulfType.NAME);
		if (start == null) {
	...
	/**
	 * Find one WorkReqFulfType
	 */
	public Optional<WorkReqFulfType> findOne(Object workReqFulfTypeId) {
		List<WorkReqFulfType> entityList = null;
		In in = new In();
		in.setEntityName(WorkReqFulfType.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("workReqFulfTypeId",
						EntityOperator.EQUALS, workReqFulfTypeId)),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	...
	public List<WorkRequirementFulfillment> getWorkRequirementFulfillments(
			WorkReqFulfType workReqFulfType, Integer start, Integer number,
			List<String> orderBy) {
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("workReqFulfTypeId",
						EntityOperator.EQUALS, workReqFulfType
