...
public class BudgetReviewBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public BudgetReviewBaseService(ExecuteFindService executeFindService) {
	 ...
	 * Find BudgetReviews
	 */
	public List<BudgetReview> find(Integer start, Integer number,
	...
	/**
	 * Find one BudgetReview
	 */
	public Optional<BudgetReview> findOne(Object budgetId,
			Object budgetReviewId, Object partyId,
			Object budgetReviewResultTypeId) {
		List<BudgetReview> entityList = null;
		In in = new In();
		in.setEntityName(BudgetReview.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays.asList(
				new EntityExpr("budgetId", EntityOperator.EQUALS, budgetId),
				new EntityExpr("budgetReviewId", EntityOperator.EQUALS,
						budgetReviewId), new EntityExpr("partyId",
						EntityOperator.EQUALS, partyId), new EntityExpr(
						"budgetReviewResultTypeId", EntityOperator.EQUALS,
						budgetReviewResultTypeId)), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
				entityList = BudgetReview.fromValues(out.getListIt()
	 ...
	 * Get party
	 */
	public Optional<Party> getParty(BudgetReview budgetReview) {
		List<Party> entityList = null;
		In in = new In();
		in.setEntityName(Party.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("partyId", EntityOperator.EQUALS,
						budgetReview.getPartyId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
				entityList = Party
	 ...
	 */
	public Optional<BudgetReviewResultType> getBudgetReviewResultType(
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("budgetReviewResultTypeId",
						EntityOperator.EQUALS, budgetReview
