...
public class ProductPriceChangeBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public ProductPriceChangeBaseService(ExecuteFindService executeFindService) {
		this.executeFindService = executeFindService;
	}

	/**
	 * Count ProductPriceChanges
	 */
	...
	/**
	 * Find ProductPriceChanges
	 */
	public List<ProductPriceChange> find(Integer start, Integer number,
	...
	/**
	 * Find one ProductPriceChange
	 */
	public Optional<ProductPriceChange> findOne(Object productPriceChangeId) {
		List<ProductPriceChange> entityList = null;
		In in = new In();
		in.setEntityName(ProductPriceChange.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("productPriceChangeId",
						EntityOperator.EQUALS, productPriceChangeId)),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	...
	public Optional<ProductPrice> getProductPrice(
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays.asList(
				new EntityExpr("productId", EntityOperator.EQUALS,
						productPriceChange.getProductId()), new EntityExpr(
						"productPriceTypeId", EntityOperator.EQUALS,
						productPriceChange.getProductPriceTypeId()),
				new EntityExpr("productPricePurposeId", EntityOperator.EQUALS,
						productPriceChange.getProductPricePurposeId()),
				new EntityExpr("currencyUomId", EntityOperator.EQUALS,
						productPriceChange.getCurrencyUomId()), new EntityExpr(
						"productStoreGroupId", EntityOperator.EQUALS,
						productPriceChange.getProductStoreGroupId()),
				new EntityExpr("fromDate", EntityOperator.EQUALS,
						productPriceChange.getFromDate())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	 ...
	 */
	public Optional<UserLogin> getChangedByUserLogin(
			ProductPriceChange productPriceChange) {
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("userLoginId", EntityOperator.EQUALS,
						productPriceChange.getChangedByUserLogin())),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
