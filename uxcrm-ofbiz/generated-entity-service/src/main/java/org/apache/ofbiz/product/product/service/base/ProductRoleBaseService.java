...
public class ProductRoleBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public ProductRoleBaseService(ExecuteFindService executeFindService) {
	 ...
	 * Find ProductRoles
	 */
	public List<ProductRole> find(Integer start, Integer number,
	...
	/**
	 * Find one ProductRole
	 */
	public Optional<ProductRole> findOne(Object productId, Object partyId,
			Object roleTypeId, Object fromDate) {
		List<ProductRole> entityList = null;
		In in = new In();
		in.setEntityName(ProductRole.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("productId", EntityOperator.EQUALS,
						productId), new EntityExpr("partyId",
	 ...
	 * Get party
	 */
	public Optional<Party> getParty(ProductRole productRole) {
		List<Party> entityList = null;
		In in = new In();
		in.setEntityName(Party.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("partyId", EntityOperator.EQUALS,
						productRole.getPartyId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
				entityList = Party
	 ...
	 * Get party role
	 */
	public Optional<PartyRole> getPartyRole(ProductRole productRole) {
		List<PartyRole> entityList = null;
		In in = new In();
		in.setEntityName(PartyRole.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays.asList(
				new EntityExpr("partyId", EntityOperator.EQUALS, productRole
						.getPartyId()), new EntityExpr("roleTypeId",
						EntityOperator.EQUALS, productRole.getRoleTypeId())),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	 ...
	 * Get product
	 */
	public Optional<Product> getProduct(ProductRole productRole) {
		List<Product> entityList = null;
		In in = new In();
		in.setEntityName(Product.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("productId", EntityOperator.EQUALS,
						productRole.getProductId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
