...
public class UnemploymentClaimBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public UnemploymentClaimBaseService(ExecuteFindService executeFindService) {
		this.executeFindService = executeFindService;
	}

	/**
	 * Count UnemploymentClaims
	 */
	...
	/**
	 * Find UnemploymentClaims
	 */
	public List<UnemploymentClaim> find(Integer start, Integer number,
	...
	/**
	 * Find one UnemploymentClaim
	 */
	public Optional<UnemploymentClaim> findOne(Object unemploymentClaimId) {
		List<UnemploymentClaim> entityList = null;
		In in = new In();
		in.setEntityName(UnemploymentClaim.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("unemploymentClaimId",
						EntityOperator.EQUALS, unemploymentClaimId)),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	...
	public Optional<Employment> getEmployment(
			UnemploymentClaim unemploymentClaim) {
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays.asList(
				new EntityExpr("roleTypeIdFrom", EntityOperator.EQUALS,
						unemploymentClaim.getRoleTypeIdFrom()), new EntityExpr(
						"roleTypeIdTo", EntityOperator.EQUALS,
						unemploymentClaim.getRoleTypeIdTo()),
				new EntityExpr("partyIdFrom", EntityOperator.EQUALS,
						unemploymentClaim.getPartyIdFrom()),
				new EntityExpr("partyIdTo", EntityOperator.EQUALS,
						unemploymentClaim.getPartyIdTo()),
				new EntityExpr("fromDate", EntityOperator.EQUALS,
						unemploymentClaim.getFromDate())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	...
	public Optional<StatusItem> getStatusItem(
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("statusId", EntityOperator.EQUALS,
						unemploymentClaim.getStatusId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
