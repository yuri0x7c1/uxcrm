...
public class DeductionBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public DeductionBaseService(ExecuteFindService executeFindService) {
	 ...
	 * Find Deductions
	 */
	public List<Deduction> find(Integer start, Integer number,
	 ...
	 * Find one Deduction
	 */
	public Optional<Deduction> findOne(Object deductionId) {
		List<Deduction> entityList = null;
		In in = new In();
		in.setEntityName(Deduction.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("deductionId", EntityOperator.EQUALS,
						deductionId)), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
				entityList = Deduction.fromValues(out.getListIt()
	 ...
	 * Get deduction type
	 */
	public Optional<DeductionType> getDeductionType(Deduction deduction) {
		...
		in.setEntityConditionList(new EntityConditionList<>(
				Arrays.asList(new EntityExpr("deductionTypeId",
						EntityOperator.EQUALS, deduction.getDeductionTypeId())),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
