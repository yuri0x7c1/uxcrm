...
public class ProtectedViewBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public ProtectedViewBaseService(ExecuteFindService executeFindService) {
		this.executeFindService = executeFindService;
	}

	/**
	 * Count ProtectedViews
	 */
	 ...
	 * Find ProtectedViews
	 */
	public List<ProtectedView> find(Integer start, Integer number,
	...
	/**
	 * Find one ProtectedView
	 */
	public Optional<ProtectedView> findOne(Object groupId, Object viewNameId) {
		List<ProtectedView> entityList = null;
		In in = new In();
		in.setEntityName(ProtectedView.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("groupId", EntityOperator.EQUALS,
						groupId), new EntityExpr("viewNameId",
						EntityOperator.EQUALS, viewNameId)), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
				entityList = ProtectedView.fromValues(out.getListIt()
	 ...
	 * Get security group
	 */
	public Optional<SecurityGroup> getSecurityGroup(ProtectedView protectedView) {
		List<SecurityGroup> entityList = null;
		In in = new In();
		in.setEntityName(SecurityGroup.NAME);
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("groupId", EntityOperator.EQUALS,
						protectedView.getGroupId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	...
	public List<SecurityGroupPermission> getSecurityGroupPermissions(
			ProtectedView protectedView, Integer start, Integer number,
			List<String> orderBy) {
