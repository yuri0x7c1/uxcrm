...
public class EftAccountBaseService {

	protected ExecuteFindService executeFindService;

	@Autowired
	public EftAccountBaseService(ExecuteFindService executeFindService) {
	 ...
	 * Find EftAccounts
	 */
	public List<EftAccount> find(Integer start, Integer number,
	 ...
	 * Find one EftAccount
	 */
	public Optional<EftAccount> findOne(Object paymentMethodId) {
	 ...
	 * Get payment method
	 */
	public Optional<PaymentMethod> getPaymentMethod(EftAccount eftAccount) {
		List<PaymentMethod> entityList = null;
		In in = new In();
		in.setEntityName(PaymentMethod.NAME);
		in.setEntityConditionList(new EntityConditionList<>(
				Arrays.asList(new EntityExpr("paymentMethodId",
						EntityOperator.EQUALS, eftAccount.getPaymentMethodId())),
				EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	 ...
	 * Get contact mech
	 */
	public Optional<ContactMech> getContactMech(EftAccount eftAccount) {
		...
		in.setEntityConditionList(new EntityConditionList<>(Arrays
				.asList(new EntityExpr("contactMechId", EntityOperator.EQUALS,
						eftAccount.getContactMechId())), EntityOperator.AND));
		Out out = executeFindService.runSync(in);
		try {
			if (out.getListIt() != null) {
	 ...
	 * Get postal address
	 */
	public Optional<PostalAddress> getPostalAddress(EftAccount eftAccount) {
	...
	public List<OrderPaymentPreference> getOrderPaymentPreferences(
			EftAccount eftAccount, Integer start, Integer number,
			List<String> orderBy) {
	 ...
	 * Get payments
	 */
	public List<Payment> getPayments(EftAccount eftAccount, Integer start,
