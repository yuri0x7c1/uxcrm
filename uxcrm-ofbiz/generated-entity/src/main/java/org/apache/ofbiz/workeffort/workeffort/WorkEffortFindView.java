...
public class WorkEffortFindView implements Serializable {
	...
	public enum Fields {
		workEffortParentId, partyId, fixedAssetId, workEffortId, recurrenceInfoId, workEffortTypeId, reservPersons, revisionNumber, tempExprId, showAsEnumId, infoUrl, universalId, locationDesc, createdByUserLogin, actualMilliSeconds, quantityToProduce, workEffortPurposeTypeId, serviceLoaderName, accommodationSpotId, estimatedStartDate, sendNotificationEmail, noteId, quantityRejected, priority, currentStatusId, runtimeDataId, estimatedMilliSeconds, specialTerms, timeTransparency, actualCompletionDate, reserv2ndPPPerc, totalMoneyAllowed, estimateCalcMethod, description, moneyUomId, reservNthPPPerc, workEffortName, lastModifiedByUserLogin, accommodationMapId, actualStartDate, scopeEnumId, quantityProduced, facilityId, estimatedSetupMillis, lastModifiedDate, sourceReferenceId, lastStatusUpdate, percentComplete, totalMilliSecondsAllowed, createdDate, actualSetupMillis, estimatedCompletionDate
	}

	public WorkEffortFindView(GenericValue value) {
	...
	public static WorkEffortFindView fromValue(
			org.apache.ofbiz.entity.GenericValue value) {
		return new WorkEffortFindView(value);
	}

	public static List<WorkEffortFindView> fromValues(List<GenericValue> values) {
		List<WorkEffortFindView> entities = new ArrayList<>();
		for (GenericValue value : values) {
			entities.add(new WorkEffortFindView(value));
		}
		return entities;
	}
}